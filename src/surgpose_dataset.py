...
class SurgPoseBase(Dataset):
    """Base class containing shared logic for SurgPose datasets."""
    def __init__(self, img_root, ann_root, input_size=(256, 192), heatmap_size=(64, 48),
                 num_joints=14, sigma=2, transform=None):
    ...
    def _prepare_video_ids(self, video_list):
        # test if there is a given video list (generated from the video level data split file)
        if video_list is None:
            return sorted(os.listdir(self.img_root))
        return sorted([vid for vid in video_list if os.path.isdir(os.path.join(self.img_root, vid))])

    def generate_target(self, joints, joints_vis):
        """Gaussian heatmap generation logic"""
...
class SurgPoseDataset(SurgPoseBase):
    """Specific class for training HRNet on full SurgPose frames"""
    # def __init__(self, img_root, ann_root, input_size=(256, 192), heatmap_size=(64, 48),
    #              num_joints=14, sigma=2, transform=None, video_list=None, keep_kpt_gt= False):
    def __init__(self, *args, video_list=None, keep_kpt_gt=False, **kwargs):
        super().__init__(*args, **kwargs)
       
        self.keep_kpt_gt = keep_kpt_gt 
        video_ids=self._prepare_video_ids(video_list)
        
        # get images and keypoints
        self.samples = []
        for video_id in video_ids:
            img_dir = os.path.join(self.img_root, video_id)
            kp_dir = os.path.join(self.ann_root, video_id)
    ...
    def __getitem__(self, idx):
        ...
        #keep the GT keypoints coordinates for evaluation metrics computing
        if self.keep_kpt_gt:
            joints_gt = joints.copy()
        ...
        if self.keep_kpt_gt:
            ...
            return img, torch.from_numpy(target), torch.from_numpy(target_weight), torch.zeros(1)


class SurgPoseDatasetOneInstance(SurgPoseBase):
    """Specific class for training HRNet on unique instrument instances (crops) from SurgPose frames"""
    def __init__(self, *args, video_list=None, **kwargs):
        super().__init__(*args, **kwargs)
        num_total = 0
        num_skipped = 0
        video_ids= self._prepare_video_ids(video_list)
