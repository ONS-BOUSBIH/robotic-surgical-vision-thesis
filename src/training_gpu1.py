...
from finetuning_utils import WeightedJointsMSELoss
from dataset_preprocessing import video_level_split
import os
import torch
import numpy as np
...
def train(model, dataloader, optimizer, criterion, device):
    model.train()
    total_loss=0
    for images, targets, weights in tqdm(dataloader, desc="Training", unit="batch" ):
...
def validate(model,dataloader,criterion, device):
    ...
    with torch.no_grad():
        total_loss = 0
        for images, targets, weights in tqdm(dataloader, desc="Validation", unit="batch" ):
            images= images.to(device)
            targets= targets.to(device)
            weights = weights.to(device)
...
def main():
    ...
    # in_height, in_width= cfg.MODEL.IMAGE_SIZE[1],cfg.MODEL.IMAGE_SIZE[0]
    # sigma=cfg.MODEL.SIGMA
    # logger.info(f'input_size: {in_height}, {in_width}')
    ...
    # in_height, in_width= cfg.MODEL.IMAGE_SIZE[1],cfg.MODEL.IMAGE_SIZE[0]
    # H_h, W_h= cfg.MODEL.HEATMAP_SIZE[1],cfg.MODEL.HEATMAP_SIZE[0]
    ...
    train_dataset = SurgPoseDataset(frames_dir,keypoints_dir,video_list=train_video_list, input_size=(in_width,in_height), heatmap_size=(W_h,H_h),transform=train_transforms,sigma=sigma)
    val_dataset = SurgPoseDataset(frames_dir,keypoints_dir,video_list=val_video_list,input_size=(in_width,in_height), heatmap_size=(W_h,H_h), sigma=sigma)
